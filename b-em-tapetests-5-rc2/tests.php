<?php
  ...
  // DO NOTE that for now, your B-Em config file needs to select a sane default model configuration:
  //   Model B, no sideways RAM, OS 1.2, 8271
  ...
  define ("TESTS_VERSION", "5");
  define ("TESTS_DATE", "6th July 2025");
  ...
  define ("PATH_WAV_OPFILE",            PATH_TO_TMPDIR.$path_sep."my.wav");       // 5.0
...
*/
  ...
  run ($st, $d, $p."020__S5000_F6000__DCD-tests-for-replacement-ula-1.uef", "", $grp, X_BP0, 3, "DCD behaviour", $skip, array(), $xs3);
...
//  $d = "b 4000\nbx 0\npaste *TAPE|MCH.\"A\"|M\nc\n\n";
  run ($st, $d, $p."002_long.tibetz",  "-fasttape -tapeskip", $grp, X_BP0, 3, "-fasttape -tapeskip w/TIBET (thx. Sazhen86)",  $skip, array(), $xs2);

  // 4.2: moved into simpleslow from simple
  $d = "b 5000\nb fa18\nb fabb\nb 4000\nbx 0\nbx 1\nbx 2\nbx 3\npaste ?&4000=&12|M?&5000=&12|MCH.\"CLKSP2\"|MCALL &4000\nc\n\n";
  // TOHv4.1: bugfixed
  run ($st, $d, "", "-disc ".$p."001__S5000_F4000__clocksp2.ssd", $grp, X_BP0, 1, "CLOCKSP2", $skip, array(), $xs3);
...
// die();
  ...
  print_report_update_successes($st, (FALSE !== @strpos($g, FILEMAGIC_TIBET)), "magic ok", "no magic", "Generate TIBETZ, check magic", $grp, $skip); //, $xs2);
  
  // save WAV, obtain its contents in $g
//  $g = run_without_testing ($st, "", "", "-tapesave ".PATH_WAV_OPFILE, 0, PATH_WAV_OPFILE, $skip, $xs2);
  $g = run_without_testing ($st, "", "", "-tapesave ".PATH_WAV_OPFILE, 0, PATH_WAV_OPFILE, $skip, $xs2);
  print_report_update_successes($st, (FALSE !== @strpos($g, FILEMAGIC_WAV)), "magic ok", "no magic", "Generate WAV, check magic", $grp, $skip); //, $xs2);
  
  // make sure WAV export fails for 5 hours of audio
  // (WAV size limit is ~1GB, which is about 3h9m)
  $g = run_without_testing ($st, "", PATH_TO_TAPE_TESTS.$sep.PATH_SUBSET_SIMPLE.$sep."019__CHKCAT__5hour100.uef", "-tapesave ".PATH_WAV_OPFILE, 7, PATH_WAV_OPFILE, $skip, $xs);
  ...
  $g  = run_without_testing ($st, "b 4000\nbx 0\npaste *TAPE|M10 MODE 7|MSAVE\"A\"|M|MCALL &4000|M\nc\n\n", "", "-record -tapesave ".PATH_WAV_OPFILE, 0, PATH_WAV_OPFILE, $skip, $xs2);
  $g2 = run_without_testing ($st, "b 4000\nbx 0\npaste *TAPE|M10 MODE 7|MSAVE\"A\"|M|MCALL &4000|M\nc\n\n", "", "-tapewavcosine -record -tapesave ".PATH_WAV_OPFILE, 0, PATH_WAV_OPFILE, $skip, $xs2);
  print_report_update_successes($st, $g!=$g2, "no match", "match", "Check -tapewavcosine does something", $grp, $skip); //, $xs2);
