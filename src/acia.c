...
static int mc6850_receive(ACIA * const acia, uint8_t const byte);
...
int acia_poll_rxc (ACIA    * const acia,
                   bool    * const fire_rxc_out,
                   int32_t * const divider_out) {

    /* TOHv4.3: now in a function */
    *divider_out = acia_get_divider_from_ctrl_reg_bits(3 & acia->control_reg);
    if (0 == *divider_out) {
...
/* call with (3 & acia->control_reg), or similar for TX */
int32_t acia_get_divider_from_ctrl_reg_bits (uint8_t const cr3) {

    if (0x0 == cr3) {
        return 1;     /* if tape read (ULA /64) then 19200 baud */
    } else if (0x1 == cr3) {
        return 16;    /* if tape read (ULA /64) then  1200 baud */
    } else if (0x2 == cr3) {
        return 64;    /* if tape read (ULA /64) then   300 baud */
    }
    return 0; /* have fun dividing by this */

}

void acia_get_framing (uint8_t const ctrl_reg, serial_framing_t * const f) {
...
static int acia_tx_shift (ACIA *acia, serial_framing_t *f) {
    ...
    /* TOHv4.1: let's try that again */
    total_bits = f->num_data_bits + f->num_stop_bits + ((f->parity == 'N') ? 0 : 1);

    if (acia->tx_shift_reg_shift < total_bits) {
...
int acia_savestate_BEMSNAP4(ACIA *acia, FILE *f)
{
    ...
    bytes[26] = acia->tx_odd_ones; /* TOHv4.1: parity for TX operations */
...
int acia_loadstate_BEMSNAP4(ACIA *acia, FILE *f)
{
    ...
    sn = (int8_t) bytes[11];
    if (    (sn!=k_mc6850_state_null)
         && (sn!=k_mc6850_state_need_start)
         && (sn!=k_mc6850_state_need_data)
         && (sn!=k_mc6850_state_need_parity)
         && (sn!=k_mc6850_state_need_stop)) {
    ...
    if (bytes[26]>1) {
        log_warn("acia: load state: state is corrupt @ tx_odd_ones");
        return TAPE_E_LOADSTATE;
    }
    ...
    acia->tx_odd_ones            =           bytes[26] ? 1 : 0; /* TOHv4.1 */
...
void acia_hack_tx_reset_shift_register (ACIA *acia) {
    /*log_warn("WARNING! acia_hack_tx_reset_shift_register()");*/
    reset_tx_shift_register(acia);
}

static void reset_tx_shift_register(ACIA *acia) {
    acia->tx_shift_reg_shift   = 0;
    acia->tx_shift_reg_loaded  = 0;
    acia->tx_shift_reg_value   = 0; /* TOHv3.2 */
}

int acia_receive_bit (ACIA * const acia, uint8_t const bit) {
  return mc6850_receive_bit (acia, bit);
}

/* code must be '1', '0', 'L' or 'S' */
...
uint8_t acia_rx_awaiting_start (ACIA *acia) {
  return (k_mc6850_state_need_start == acia->state);
}

uint8_t acia_rx_frame_ready (ACIA *acia) {
  return (k_mc6850_state_need_stop == acia->state);
}
...
/* TOHv4.1: option to deliberately fail the PBS protection
 *          (for proving the tests) */
/*#define HACK_DELIBERATELY_BREAK_PRO_BOXING_SIMULATOR*/

#ifdef HACK_DELIBERATELY_BREAK_PRO_BOXING_SIMULATOR
static int hack_delay_tdre=0;
#endif
...
#ifdef HACK_DELIBERATELY_BREAK_PRO_BOXING_SIMULATOR
        /* introduce long, artificial TDRE delay (TXC here is clocked quite fast I think) */
        if (hack_delay_tdre < 25) {
            hack_delay_tdre++;
        } else {
            acia->tx_waiting_to_set_tdre =  0;
            acia->status_reg             |= k_serial_acia_status_TDRE;
...
#else
        /* normal behaviour: one halfbit of TDRE delay */
        acia->tx_waiting_to_set_tdre =  0;
        acia->status_reg             |= k_serial_acia_status_TDRE;
        mc6850_update_irq_and_status_read(acia);
#endif
...
static int
mc6850_receive(ACIA * const acia, uint8_t const byte) {
