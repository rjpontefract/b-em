...
/* TOHv4.4 */
static int interval_mark (tape_interval_t * const ivl);

static void finish_marker (tape_interval_marker_t *m) {
    if (NULL == m) { return; }
    /* TODO: free actual text, once this is implemented */
    m->pos_1200ths = 0;
}
...
void tape_interval_list_finish (tape_interval_list_t * const iv_list_inout) {
    ...
    for (i=0; i < iv_list_inout->fill; i++) {
        if (NULL != iv_list_inout->list[i].markers) {
            finish_marker(iv_list_inout->list[i].markers + i);
        }
    }
...
int tape_interval_list_clone (tape_interval_list_t * const out,
    ...
    if (in->alloc > 0) {
        if (in->fill > in->alloc) { /* TOHv4.3-a3 sanity */
            log_warn("tape-interval: clone interval list: BUG: in->fill(%d) > in->alloc(%d)", in->fill, in->alloc);
            return TAPE_E_MALLOC;
        }
        tmp = malloc(in->alloc * sizeof(tape_interval_t));
        if (NULL == tmp) {
            log_warn("tape-interval: ERROR: out of memory cloning intervals list");
            return TAPE_E_MALLOC;
        }
    ...
    /* TODO: will need to clone the text in the markers in the list too */
    out->alloc = in->alloc;
    out->fill  = in->fill;
    out->list  = tmp;
    return TAPE_E_OK;
...
int tape_interval_list_append (tape_interval_list_t  * const iv_list_inout,
    ...
    if (NULL == iv_list_inout) {
        log_warn("tape-interval: BUG: append to intervals list: iv_list_inout is NULL");
        return TAPE_E_BUG;
    }
    ...
    if (iv_list_inout->alloc <= iv_list_inout->fill) {
        ...
        if (NULL == tmp) {
            log_warn("tape-interval: initial scan: appending to interval list: out of memory");
            return TAPE_E_MALLOC;
        }
...
int tape_interval_list_send_1200th (tape_interval_list_t * const iv_list_inout,
                                    ...
                                    int32_t const num_markers) { /* TOHv4.4 */
    int e;
    int32_t i;
    ...
    if ( ! TAPE_TONECODE_IS_LEGAL(tone) ) {
        log_warn("tape-interval: BUG: send_1200th_to_interval_list: bad tone (&%x)", tone);
        return TAPE_E_BUG;
    }
#endif
    ...
    for (i=0; (TAPE_E_OK == e) && (i < num_markers); i++) {
        e = interval_mark (&(iv_list_inout->decstate.wip));
    }
...
static int interval_mark (tape_interval_t * const ivl) {
    size_t alloc_new;
    tape_interval_marker_t *markers_new;
    if (ivl->num_markers_fill >= ivl->num_markers_alloc) {
        alloc_new = ivl->num_markers_fill + 10;
        markers_new = realloc(ivl->markers,
                                       sizeof(tape_interval_marker_t) * alloc_new);
        if (NULL == markers_new) {
            log_warn("tape-interval: out of memory allocating tape interval markers list");
            return TAPE_E_MALLOC;
        }
        ivl->num_markers_alloc = alloc_new;
        ivl->markers = markers_new;
    }
    memset(ivl->markers + ivl->num_markers_fill,
           0,
           sizeof(tape_interval_marker_t));
    /* copy the current interval time tally onto the marker */
    ivl->markers[ivl->num_markers_fill].pos_1200ths = ivl->pos_1200ths; 
    (ivl->num_markers_fill)++;
    return TAPE_E_OK;
} 
