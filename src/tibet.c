...
static int c_next_tonechar (tibet_priv_t *tp,
                            uint8_t populate_elapsed,
                            char *tc,
                            /* TOHv4.4: bugfix: */
                            int32_t *elapsed_1200ths_or_minus_one_out);
static int c_read_1200 (tibet_priv_t *tp,
                        char *stray_2400th_inout,
                        //~ char *tonechar_pushed_back_inout, // one pushed-back tonechar, or '\0' means empty
                        uint8_t populate_elapsed,
                        uint8_t *out_or_null,
                        uint32_t *cur_span_out,
                        int32_t *elapsed_1200ths_or_null_or_minus_one_out);
static int c_get_priv (tibet_t *t, tibet_priv_t **priv, uint8_t enforce_decoded);
static int c_get_span (tibet_priv_t *tp,
                       uint8_t populate_elapsed,
                       tibet_span_t **span,
                       /* TOHv4.4: bugfix: */
                       int32_t *elapsed_1200ths_or_minus_one_out);
...
enforce_hint_compat_for_silent_spans (tibet_span_hints_t *active_hints,
  ...
  // /time is always permitted, and /baud is too, now (as of 2.0.5)
  // silent spans: reject (framing, /speed, /phase)

  // 2.0.5: relaxed: overzealous, since baud is persistent
  // (and to post-hoc fix 07__Agfa_Gevaert__SideA.wav.tibet from VectorEyes Set 1)

  // if (active_hints->have_baud) {
  //   fields_msg = TIBET_K_BAUD;
  // } else
  if (active_hints->have_framing) {
...
static int c_next_tonechar (tibet_priv_t *tp,
  ...
  e = c_get_span(tp, populate_elapsed, &span, elapsed_1200ths_or_minus_one_out);
  if (TAPE_E_OK != e) { return e; }

//~ printf("c_next_tonechar: stray_2400th_in = %c, ", *stray_2400th_inout);

  if (TIBET_SPAN_DATA == span->type) {
    //~ if ('\0' != *stray_2400th_inout) {
      //~ *tc = *stray_2400th_inout;
    //~ } else {
    if ('P' == span->tones[tp->c_tone_pos]) {
      // we just skip pulses
      (tp->c_tone_pos)++;
      next_tonechar_do_elapsed(populate_elapsed, span);
      return c_next_tonechar (tp, populate_elapsed, tc, elapsed_1200ths_or_minus_one_out); //, stray_2400th_inout); // and recurse
    }
    ...
    *tc = (span->tones[tp->c_tone_pos] == '.') ? '1' : '0';
    (tp->c_tone_pos)++;
    //~ }
    //~ *stray_2400th_inout = '\0';
  } else if (TIBET_SPAN_SILENT == span->type) {
    *tc = 'S';

    /* which of these do we want to do ... ? */
    
//~ tp->c_silence_pos_s += (832.0 / 2000000.0);

    /* TOHv4.4: if a stray 2400th is left over from a squawk block,
     * spend this first before beginning to consume time from the actual
     * silent block's reserve of tapetime. */
    ...
    //~ if ( '\0' == *stray_2400th_inout ) {
        ...
        tp->c_silence_pos_s += (832.0 / 2000000.0);
    //~ } else {
        //~ log_warn("TIBET: WARNING: stray 2400th into silence, value %c\n", *stray_2400th_inout);
    //~ }
    //~ *stray_2400th_inout = '\0';
...
// printf("\n\n");
    *tc = 'L';
    /* TOHv4.4: if a stray 2400th is left over from a squawk block,
     * spend this first before beginning to consume time from the actual
     * leader block's reserve of tapetime. */
    //~ if ( '\0' == *stray_2400th_inout ) {
        (tp->c_tone_pos)++;
    //~ } else {
        /*log_warn("TIBET: WARNING: stray 2400th into leader, value %c\n", *stray_2400th_inout);*/
    //~ }
    //~ *stray_2400th_inout = '\0';
  }
  ...
  // note that this has an internal resolution in 2400ths
  e = c_get_span(tp, populate_elapsed, &span, elapsed_1200ths_or_minus_one_out); //, stray_2400th_inout);  
  
//~ printf("stray_2400th_out = %c\n", *stray_2400th_inout);
//~ printf("tc = %c\n", *tc);
...
static void next_tonechar_do_elapsed (bool const populate_elapsed, tibet_span_t * const span) {
  ...
  //~ printf("next_tonechar_do_elapsed: span %d, type %u: pos_1200ths=%d, sub_pos_4800ths=%d\n", span->id, span->type, span->timespan.pos_1200ths, span->timespan.sub_pos_4800ths);
}
...
static int c_get_span (tibet_priv_t *tp,
  ...
  *span = NULL;
  ...
  // printf("dd_new_spans[%d+1].start_1200ths = %d\n", tp->c_cur_span, t0v);
        ...
        t1->pos_1200ths     = 0;
        t1->sub_pos_4800ths = t0->sub_pos_4800ths; /* TOHv4.4 */
      }
    }
    (tp->c_cur_span)++;
    tp->c_tone_pos = 0;
    tp->c_silence_pos_s = 0.0f;
    if (NULL != elapsed_1200ths_or_minus_one_out) {
      *elapsed_1200ths_or_minus_one_out = t0v;
    }
  }
...
// *out_or_null will be set to '0', '1', 'L', or 'S'
...
static int c_read_1200 (tibet_priv_t *tp,
                        ...
                        uint8_t *out_or_null,
                        uint32_t *cur_span_out,
                        int32_t *elapsed_1200ths_or_minus_one_or_null_out) {
  ...
  *cur_span_out = 0;
  ...
  e = c_get_span(tp, populate_elapsed, &span, elapsed_1200ths_or_minus_one_or_null_out);
  if (TAPE_E_OK != e) { return e; }
  
  if ('\0' != *stray_2400th_inout) {
    // then try this
    tc1 = *stray_2400th_inout;
    *stray_2400th_inout = '\0';
  } else {
    e = c_next_tonechar (tp, populate_elapsed, &tc1, elapsed_1200ths_or_minus_one_or_null_out); //, stray_2400th_inout);
    if (TAPE_E_OK != e) { return e; }
  }

  e = c_next_tonechar (tp, populate_elapsed, &tc2, elapsed_1200ths_or_minus_one_or_null_out); //, stray_2400th_inout);
  if (TAPE_E_OK != e) { return e; }
  ...
  do {
    ...
    *cur_span_out = tp->c_cur_span;
    ...
    /* TOHv4.4: account for the missing 2400th of tape time which went missing */
    if ('\0' != *stray_2400th_inout) {
        *stray_2400th_inout = '\0';
        if (NULL != out_or_null) {
            *out_or_null = '1'; /* two stray 1200ths add up to one 1200th; ambiguous value, use '1' */
            /* we already pulled another tonechar, so re-use it next time we're called */
            *stray_2400th_inout = tc2;
        }

        break;
    } else {
        *stray_2400th_inout = tc1;
    }
...
int tibet_read_tonechar (tibet_t *t,
                         uint8_t populate_elapsed,
                         char *value_out,
                         char *stray_2400th_inout,
                         int32_t *elapsed_1200ths_or_null_or_minus_one_out) {
  ...
  e = c_get_span (tp, populate_elapsed, &span, elapsed_1200ths_or_null_or_minus_one_out);
  if (TAPE_E_OK != e) { return e; }

  e = c_next_tonechar (tp, populate_elapsed, &tc, elapsed_1200ths_or_null_or_minus_one_out);
  if (TAPE_E_OK != e) { return e; }
...
int tibet_read_bit (tibet_t *t,
                    char *stray_2400th_inout, /* TOHv4.4 */
                    uint8_t baud300,
                    uint8_t populate_elapsed,         /* TOHv4.3 */
                    char *bit_out_or_null,
                    int32_t *elapsed_1200ths_or_minus_one_or_null_out) /* TOHv4.3 */ {
  ...
  for (n=0; n < pairs_to_read; n++) {
    uint8_t tone, tval;
    e = c_read_1200 (tp, stray_2400th_inout, populate_elapsed, &tone, &cur_span, elapsed_1200ths_or_minus_one_or_null_out);
    if (TAPE_E_OK != e) { return e; }
    if ('L' == tone) {
  ...
  //~ if (NULL != elapsed_1200ths_or_minus_one_or_null_out) {
    //~ printf("tibet_read_bit: elapsed_1200ths = %d\n", *elapsed_1200ths_or_minus_one_or_null_out);
  //~ }
...
// printf("append_leader_span: start 1200ths %d, num 2400ths %u\n", start_1200ths, num_2400ths);
  ...
  span->hints            = *hints;
  span->num_tones        = num_2400ths; // & ~1; /* TOHv4.4: avoid 1/2400th discrepancy */
  span->type             = TIBET_SPAN_LEADER;
  //~ span->id = (tp->dd_new_num_spans - 1);
  memset(hints, 0, sizeof(tibet_span_hints_t));
  /* TOHv4.3 */
  if (start_1200ths > 0) {
    span->timespan.start_1200ths   = start_1200ths;
    span->timespan.pos_1200ths     = num_2400ths / 2;
    span->timespan.sub_pos_4800ths = (num_2400ths << 1) & 3;
  }
  return TAPE_E_OK;
}
...
//~ printf("tibet_append_data_span: %d + %d = %d\n", span->timespan.start_1200ths, span->timespan.pos_1200ths, span->timespan.start_1200ths+span->timespan.pos_1200ths);
  ...
  span->id = (priv->dd_new_num_spans - 1); /* TOHv4.4: fix missing span ID */
