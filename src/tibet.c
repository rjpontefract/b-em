/* TIBET reference decoder, v2.1
   'Diminished', June 2024
   Public Domain */
...
static int append_silent_span (tibet_priv_t *tp, float len_s, tibet_span_hints_t *hints);
static int append_leader_span (tibet_priv_t *tp, uint32_t num_2400ths, tibet_span_hints_t *hints);
...
static int parse_leader (tibet_priv_t *tp, char *v, tibet_span_hints_t *active_hints);
...
static int c_next_tonechar (tibet_priv_t *tp, char *tc);
static int c_read_1200 (tibet_priv_t *t, uint8_t *out, uint32_t *cur_span_out);
static int c_get_priv (tibet_t *t, tibet_priv_t **priv, uint8_t enforce_decoded);
static int c_get_span (tibet_priv_t *tp, tibet_span_t **span);
...
static int parse_leader (tibet_priv_t *tp, char *v, tibet_span_hints_t *active_hints) { //}, tibet_span_t *pending_span) {
  int e;
  uint32_t i;
  /* overhaul v3: add check for missing field */
  if (NULL == v) {
  ...
  e = append_leader_span(tp, i, active_hints);
  return e;
}
...
static int parse_silence (tibet_priv_t *tp, char *v, tibet_span_hints_t *active_hints) {
  ...
  */
  ...
  // work out a number of atoms equivalent to this silent length
  // we're going to ignore the speed hint and just assume a speed of 1.0
  //span->num_tones = (uint32_t) roundf(f * TIBET_FREQ_HI);
  e = append_silent_span (tp, f, active_hints); //(uint32_t) roundf(f * TIBET_FREQ_HI));
  return e;
}
...
//printf("num_tones = %u, num_tones_alloc = %u\n", span->num_tones, span->num_tones_alloc);
...
static int append_tonechar (tibet_priv_t *tp, char c) {
  //int e;
  tibet_span_t *span;
  char *p;
  uint32_t newlen;
  ...
  if (span->num_tones >= SPAN_MAX_TONES) {
    TIBET_ERR ("TIBET: line %u: too many tone characters in span\n", tp->ds_linenum + 1);
    return TAPE_E_TIBET_DATA_EXCESSIVE_TONES;
  }
  if ((span->num_tones > 0) && ('P' == c) && ('P' == span->tones[span->num_tones - 1])) {
    TIBET_ERR ("TIBET: line %u: illegal double pulse PP\n",
  ...
  if (span->num_tones >= span->num_tones_alloc) {
    newlen = span->num_tones_alloc + SPAN_TONES_ALLOC_DELTA;
    p = TIBET_REALLOC(span->tones, newlen);
    if (NULL == p) {
      TIBET_ERR ("TIBET: line %u: could not realloc tones\n", tp->ds_linenum + 1);
      return TAPE_E_MALLOC;
    }
    span->tones = p;
    span->num_tones_alloc = newlen;
...
static int c_next_tonechar (tibet_priv_t *tp, char *tc) {
  ...
  if (TIBET_SPAN_DATA == span->type) {
    if (span->tones[tp->c_tone_pos] == 'P') {
      // we just skip pulses
      (tp->c_tone_pos)++;
      return c_next_tonechar (tp, tc); // and recurse
    }
...
static int c_read_1200 (tibet_priv_t *tp, uint8_t *out, uint32_t *cur_span_out) {
  ...
  *cur_span_out = 0;
  ...
  e = c_next_tonechar (tp, &tc1);
  if (TAPE_E_OK != e) { return e; }
  e = c_next_tonechar (tp, &tc2);
  if (TAPE_E_OK != e) { return e; }
...
#define TIBET_OP_FMT_DATA_PFX "%s\n"
        ...
        for (tn = 0; tn < span->num_tones; tn++) {
          ...
          pos++;
          if (((tn % 20) == 19) && (tn < (span->num_tones - 1))) {
            if (1 == pass) { buf[pos] = '\n'; }
            pos++;
          }
        }
...
int tibet_read_bit (tibet_t *t, uint8_t baud300, char *bit_out) {
  ...
  for (n=0; n < pairs_to_read; n++) {
    uint8_t tone, tval;
    e = c_read_1200 (tp, &tone, &cur_span);
    if (TAPE_E_OK != e) { return e; }
    if (tone == 'L') {
...
int tibet_clone (tibet_t *out, tibet_t *in) {
  ...
  for (n=0; n < tp_in->dd_new_num_spans; n++) {
    tibet_span_t *span;
    span = tp_out->dd_new_spans + n;
    if (tp_in->dd_new_spans[n].tones != NULL) {
      span->tones = malloc(span->num_tones);
      if (NULL == span->tones) {
        TIBET_ERR("TIBET: could not malloc clone span tones\n");
        tibet_finish(out);
        return TAPE_E_MALLOC;
      }
      memcpy(span->tones, tp_in->dd_new_spans[n].tones, span->num_tones);
    } else {
      span->tones = NULL;
    }
  }
...
int tibet_append_leader_span (tibet_t *t, uint32_t num_2400ths, tibet_span_hints_t *hints) {
  ...
  e = c_get_priv(t, &tp, 0);
  if (TAPE_E_OK != e) { return e; }
  return append_leader_span(tp, num_2400ths, hints);
}
...
/*printf("id %u\n", span->id);*/
  //span->hints.have_speed = 1;
  //span->hints.speed      = speed;
  span->hints            = *hints;
...
int tibet_append_silent_span (tibet_t *t, float len_s, tibet_span_hints_t *hints) {
  ...
  e = c_get_priv(t, &tp, 0);
  if (TAPE_E_OK != e) { return e; }
  return append_silent_span (tp, len_s, hints);
}
...
static int append_data_span (tibet_priv_t *priv, tibet_span_t *pending) {
  ...
  if (pending->type != TIBET_SPAN_DATA) {
    log_warn("TIBET: BUG: pending span has bad type (%u)\n", pending->type);
    return TAPE_E_BUG;
  }
