...
/* TOHv4.4 */
static ALLEGRO_MENU *tape_sound_menu;
static ALLEGRO_MENU *tape_turbo_menu;
...
static ALLEGRO_MENU *create_tape_menu(void)
{
    ...
    tape_turbo_menu = al_create_menu(); /* TOHv4.4 */
    ...
    al_append_menu_item(tape_turbo_menu, "Skip leader/gaps + fast DCD", IDM_TAPE_TURBO_STRIP, fflags, NULL, NULL);
    /* updated for TOHv3.2: */
    ...
    al_append_menu_item(tape_turbo_menu, "Overclock ACIA + fast DCD (dubious)", IDM_TAPE_TURBO_OVERCLOCK, fflags, NULL, NULL);
    ...
    al_append_menu_item(menu, "Turbo load", 0, 0, NULL, tape_turbo_menu);
...
static ALLEGRO_MENU *create_sound_menu(void)
{
    ...
    add_checkbox_item(menu, "Tape signal",           IDM_SOUND_TAPE,      sound_tape_volume_fraction > TAPENOISE_VOLUME_MINIMUM);
...
static ALLEGRO_MENU *create_settings_menu(void)
{
    ...
    tape_sound_menu = create_sound_menu(); /* TOHv4.4 */
    al_append_menu_item(menu, "Sound...", 0, 0, NULL, tape_sound_menu);
#ifdef HAVE_LINUX_MIDI
...
void gui_alter_tape_eject (const char * const path) {
    ...
    if (NULL == path) {
        ...
        sfx2len = strlen(sfx2);
        len = strlen(path+z);
#define GUI_MENU_TAPEFILE_MAXLEN 79
...
static void tape_load_ui(ALLEGRO_EVENT *event) {
    gui_load_tape((ALLEGRO_DISPLAY *)(event->user.data2));
}

void gui_load_tape(ALLEGRO_DISPLAY *display)
{
    const char *fpath;
    ...
    if (chooser) {
        //~ ALLEGRO_DISPLAY *display = (ALLEGRO_DISPLAY *)(event->user.data2);
        if (al_show_native_file_dialog((ALLEGRO_DISPLAY*)display, chooser)) {
            if (al_get_native_file_dialog_count(chooser) > 0) {
                /* TOHv3 */
                ...
                ALLEGRO_PATH *path = al_create_path(al_get_native_file_dialog_path(chooser, 0));
                /* TOHv4.3: tape_handle_exception() should deal with errors, so ignore return code */
                /*e =*/ tape_load(&tape_state, &tape_vars, path);
...
int gui_save_tape (ALLEGRO_DISPLAY * display,
                   const char *ext,
                   bool as_uef,
                   bool as_csw,
                   bool as_tibet,
    ...
    /* TOHv4.4: HACK: al_create_native_file_dialog() seems to function differently
                on macOS; ext is appended to fpath automatically it seems,
                so it mustn't be pre-appended ... */
#ifdef __APPLE__
    ext_p = "";
#else
    ext_p = strchr(ext, '.');
#endif
    snprintf(name, sizeof(name), "new%s", ext_p); /* e.g. name = "new.uef" */
...
static int tape_save_ui (ALLEGRO_EVENT *event, const char *ext) {
    ...
    mid = menu_get_id(event);

    as_uef   = (IDM_TAPE_SAVE_UEF   == mid) || (IDM_TAPE_SAVE_UEF_UNCOMP == mid);
    as_csw   = (IDM_TAPE_SAVE_CSW   == mid) || (IDM_TAPE_SAVE_CSW_UNCOMP == mid);
    as_tibet = (IDM_TAPE_SAVE_TIBET == mid) || (IDM_TAPE_SAVE_TIBETZ     == mid);
    as_wav   = (IDM_TAPE_SAVE_WAV   == mid);
    compress = (IDM_TAPE_SAVE_UEF   == mid) || (IDM_TAPE_SAVE_CSW        == mid) || (IDM_TAPE_SAVE_TIBETZ == mid);

    d = (ALLEGRO_DISPLAY *)(event->user.data2);

    return gui_save_tape (d, ext, as_uef, as_csw, as_tibet, as_wav, compress);
...
void tape_toggle_turbo_overclock(void) //ALLEGRO_MENU *menu) /*ALLEGRO_EVENT *event)*/
{
    //~ ALLEGRO_MENU *menu = (ALLEGRO_MENU *)(event->user.data3);
    ALLEGRO_MENU *menu;
    menu = tape_turbo_menu;
...
#ifdef BUILD_TAPE_TAPECTRL
    tapectrl_to_gui_msg_overclock (&(tape_vars.tapectrl), /* TOHv4.4 */
                                    true,
                                    true,
                                    tape_vars.overclock);
#endif
...
/* TOHv3.2: repurposed */
void tape_toggle_turbo_strip(void) /*ALLEGRO_EVENT *event)*/
{
    //~ ALLEGRO_MENU *menu = (ALLEGRO_MENU *)(event->user.data3);
    ...
    if ( tape_vars.strip_silence_and_leader ) {
        ...
        al_set_menu_item_flags(menu, IDM_TAPE_TURBO_STRIP, ALLEGRO_MENU_ITEM_CHECKBOX);
    } else {
        tape_vars.strip_silence_and_leader = 1;
        al_set_menu_item_flags(menu, IDM_TAPE_TURBO_STRIP, ALLEGRO_MENU_ITEM_CHECKBOX|ALLEGRO_MENU_ITEM_CHECKED);
    }
    
#ifdef BUILD_TAPE_TAPECTRL
    tapectrl_to_gui_msg_strip (&(tape_vars.tapectrl), /* TOHv4.4 */
                                true,
                                true,
                                tape_vars.strip_silence_and_leader);
#endif

    /* TOHv4.4: reset this on button toggle: enable 0.6s strip holdoff: */
    tape_state.ula_motor_stopped_ticks_1200 = TAPE_MIN_TIME_MOTOR_OFF_TO_CLR_STRIP_HOLDOFF_1200THS;
    tape_state.strip_silence_and_leader_holdoff_1200ths = 0; /* TOHv4.4: reset this on button toggle */
...
/* TOHv4.4 */
void gui_set_tapenoise_menu_enabled (bool const activated) {
    if (NULL != tape_sound_menu) {
        al_set_menu_item_flags (tape_sound_menu,
                                IDM_SOUND_TAPE,
                                ALLEGRO_MENU_ITEM_CHECKBOX|(activated?ALLEGRO_MENU_ITEM_CHECKED:0));
    }
}
...
void gui_allegro_event(ALLEGRO_EVENT *event)
{
    ...
    switch(menu_get_id(event)) {
        ...
        case IDM_TAPE_TURBO_OVERCLOCK:
            tape_toggle_turbo_overclock(); //(ALLEGRO_MENU *)(event->user.data3));
            break;
        case IDM_TAPE_TURBO_STRIP:
            tape_toggle_turbo_strip(); //(ALLEGRO_MENU *)(event->user.data3));
            break;
        case IDM_TAPE_CAT:
            gui_tapecat_start();
            break;
        ...
        case IDM_SOUND_TAPE:
             /* TOHv4.4 */
            tapenoise_set_volume(tcw,
                                 sound_tape_volume_fraction >= TAPENOISE_VOLUME_MINIMUM ? 0.0f : TAPENOISE_VOLUME_DEFAULT, /* toggle */
                                 false); /* don't alter menu */
            break;
        /* TOHv2: */
