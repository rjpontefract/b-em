...
#define PI 3.14159
/* FIXME? We might need to allocate this on the heap instead;
...
/* each of these represents one or two cycles at the output sample rate,
   ...
   TOHv4.4: now all floating-point; resolution to int16_t will be done later */
static float sinewave_2400_slow_f[SLOW_SMPS]; /* a pair of 2400 cycles */
static float sinewave_2400_fast_f[FAST_SMPS];
static float sinewave_1200_slow_f[SLOW_SMPS]; /* one single 1200 cycle */
static float sinewave_1200_fast_f[FAST_SMPS];
...
static void ringbuf_play (int16_t *buf, float volume);
static void ringbuf_append (float *src, uint32_t len, uint8_t *no_emsgs_inout);

/* TOHv4.4: volume is no longer pre-multiplied into the sine tables;
 *          it's now done when the ringbuf is written */
static void init_sine_waves(void) {
    uint32_t n;
    double d;
...
/* TOHv4.4 */
void tapenoise_set_volume (tape_ctrl_window_t * const tcw, float const fraction, bool const alter_menu) {

    bool activated_next;
    
    activated_next = (fraction > TAPENOISE_VOLUME_MINIMUM);
    ...
    if (alter_menu) {
        gui_set_tapenoise_menu_enabled(activated_next);
    }
    
    sound_tape_volume_fraction = fraction; /* override default volume value that was just set */
    
    /* if tapectrl is hot, it must be notified of this change */
#ifdef BUILD_TAPE_TAPECTRL
    tapectrl_to_gui_msg_tapenoise (tcw, true, true, fraction);
#endif
...
/* note that tcw->mutex needs to exist already, before this is called */
void tapenoise_init(ALLEGRO_EVENT_QUEUE *queue, tape_ctrl_window_t *tcw)
{
    ...
    //~ tapenoise_set_volume (tcw, TAPENOISE_VOLUME_DEFAULT, /* alter menus */ true); /* TOHv4.4 */
}
...
void tapenoise_send_1200 (char tone_1200th, uint8_t *no_emsgs_inout) {
    ...
    float *sine1200_f, *sine2400_f; /* TOHv4.4: now float */
    uint32_t num_smps;
    uint8_t hysteresis;
...
/* printf("%c", tone_1200th); fflush(stdout); */
    ...
    if (('L' != tone_1200th) || ! currently_fast) {
        ...
        if (currently_fast) {
            num_smps = FAST_SMPS;
            sine1200_f = sinewave_1200_fast_f;
            sine2400_f = sinewave_2400_fast_f;
        } else {
            num_smps = SLOW_SMPS;
            sine1200_f = sinewave_1200_slow_f;
            sine2400_f = sinewave_2400_slow_f;
        }
        
        /* TOHv4.4: volume now applied at ringbuf */
        if ('0' == tone_1200th) {
            ringbuf_append(sine1200_f, num_smps, no_emsgs_inout);
        } else if (('1' == tone_1200th) || ('L' == tone_1200th)) {
            ringbuf_append(sine2400_f, num_smps, no_emsgs_inout);
        } else {
...
void tapenoise_play_ringbuf (float volume) {
    int16_t *tapebuffer;
    ...
    if (NULL != (tapebuffer = al_get_audio_stream_fragment(stream))) {
        ringbuf_play(tapebuffer, volume);
...
static void ringbuf_append (float *src, uint32_t len, uint8_t *no_emsgs_inout) {
    ...
    *no_emsgs_inout = 0; /* re-enable future error messages */
    ...
    for (n=0; n < len; n++, pos++) {
        ...
        audio_ringbuf[pos] = (int16_t) (i /* * volume_frac*/ * 32767.0f);
    }
...
static void ringbuf_play (int16_t *buf, float volume) {
    uint32_t n;
    float volume_transformed;
   
    /* remember, BUFLEN_TN is the Allegro output buffer length;
       AUDIO_RINGBUF_LEN is the internal ring buffer length. */

    /* volume control is non-linear */
    volume_transformed = volume*volume;
    ...
    for (n=0; (n < BUFLEN_TN) && (audio_ringbuf_fill > 0); n++) {
        ...
        buf[n] = (int16_t) (volume_transformed * audio_ringbuf[audio_ringbuf_start]);
        audio_ringbuf_start++;
        audio_ringbuf_fill--;
    }
...
#define WAV_HEADER_LEN 44
    ...
    /* get a copy of the live tape state */
    ...
    if (TAPE_FILETYPE_BITS_NONE == clone.filetype_bits) {
        ...
        tape_state_finish(&clone, false); /* BUGFIX: TOHv4.3 */
        return TAPE_E_BUG;
    }
    ...
    do {
        e = tape_1200th_from_back_end (false, &clone, false, false, &tone, NULL, NULL);
...
#define WAV_BODY_LIMIT_BYTES 1000000000
    if (num_1200ths > (WAV_BODY_LIMIT_BYTES / (SLOW_SMPS * 2))) {
        ...
        tape_state_finish(&clone, false);
        return TAPE_E_SAVE_WAV_BODY_TOO_LARGE;
    }
    
    e = TAPE_E_OK;

    tape_rewind_2(&clone, NULL, false, false);
