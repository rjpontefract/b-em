...
//~ printf("111: pre_2400ths = %u, post_2400ths = %u\n", c->nodata_total_pre_2400ths, c->nodata_total_post_2400ths);
    } else if (0x112==type) {
    ...
    if (cb && is_chunk_spent(c, src)) {
        ...
        */
        log_warn("uef: chunk ix %d, type &%x is somehow zero-duration? Skipping", u->cur_chunk, type);
        cb = 0; /* as you were */
    }
...
static int reload_reservoir (uef_state_t *u, uint8_t baud300) {
    ...
    if ( (0x100 == type) || (0x104 == type) ) {
        ...
        src->reservoir_is_leader = true; /* TOHv4.4; hack to allow 'L' to be played instead of '1' */
...
int uef_read_1200th (uef_state_t * const u,
    ...
    if (out_1200th_or_null != NULL) {
        if ( ! src->silence ) {
            ...
            *out_1200th_or_null = (0x1 & (src->reservoir_1200ths >> shift)) ? '1' : '0';
            if (('1'==*out_1200th_or_null) && src->reservoir_is_leader) { /* TOHv4.4 */
                *out_1200th_or_null = 'L'; /* yay */
            }
        } else {
    ...
    src->reservoir_is_leader = false; /* TOHv4.4 */
