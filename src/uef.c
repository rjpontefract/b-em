...
static int chunks_decode (uint8_t *buf,
    ...
    for (pos=0, uef->num_chunks=0, uef->num_chunks_alloc=0;
        ...
        e = uef_store_chunk (uef, buf + pos + 6, type, chunklen, pos); /* TOHv4.2: add offset */
        if (TAPE_E_OK != e) { return e; }
...
/* TOHv3: exported now */
int uef_store_chunk (uef_state_t *uef, uint8_t *buf, uint16_t type, uint32_t len, uint32_t offset) { /* TOHv4.2: add offset */
