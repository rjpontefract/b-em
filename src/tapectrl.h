...
// this has to be in here rather than tapectrl-labels.h as that is only ever included once
#define TAPECTRL_NUM_LABELS 6 //8    //(TAPECTRL_NUM_LAMPS_NEW) // + 1)
...
#define TAPECTRL_FROM_GUI_LOAD_TAPE         7 /* TOHv4.4 */
#define TAPECTRL_FROM_GUI_EJECT_TAPE        8 /* TOHv4.4 */
//~ #define TAPECTRL_FROM_GUI_TAPENOISE         9 /* TOHv4.4 */
#define TAPECTRL_FROM_GUI_REWIND_TAPE      10 /* TOHv4.4 */
#define TAPECTRL_FROM_GUI_CATALOGUE        11 /* TOHv4.4 */
#define TAPECTRL_FROM_GUI_TOGGLE_STRIP     12 /* TOHv4.4 */
#define TAPECTRL_FROM_GUI_TOGGLE_OVERCLOCK 13 /* TOHv4.4 */
#define TAPECTRL_FROM_GUI_SAVE_TAPE        14 /* TOHv4.4 */
#define TAPECTRL_FROM_GUI_TAPENOISE_VOLUME 15 /* TOHv4.4 */
...
#define TAPECTRL_TO_GUI_TAPENOISE 11 /* TOHv4.4 */
#define TAPECTRL_TO_GUI_STRIP     12 /* TOHv4.4 */
#define TAPECTRL_TO_GUI_OVERCLOCK 13 /* TOHv4.4 */
...
typedef struct tape_ctrl_msg_to_gui_s {
    ...
    union {
        ...
        float volume;   /* TAPECTRL_TO_GUI_TAPENOISE */
        bool overclock; /* TOHv4.4 */
        bool strip;     /* TOHv4.4 */
        struct {
            int32_t fill;
...
typedef struct tape_ctrl_window_s {
    ...
    bool tapenoise_activated;       /* TOHv4.4 */
    bool strip_silence_and_leader;  /* TOHv4.4 */
    bool overclock;                 /* TOHv4.4 */
    ...
    float tapenoise_volume; /* TOHv4.4 */
    bool halt_drawing;      /* TOHv4.4: macOS resize crash mitigation */

    /* EITHER thread may access these (via mutex) */
    ALLEGRO_MUTEX *mutex;
    ALLEGRO_DISPLAY * volatile display; /* doubles as the "window is open" flag */
    bool volatile shut_tapectrl_down;
    int volatile from_gui_fill;
    volatile tape_ctrl_msg_from_gui_t from_gui[TAPECTRL_MSG_QUEUE_SIZE];
    int volatile to_gui_fill;
    volatile tape_ctrl_msg_to_gui_t   to_gui[TAPECTRL_MSG_QUEUE_SIZE];
    int volatile tapectrl_error;   /* GUI-side error (thread shuts down?) */
...
void tapectrl_to_gui_msg_strip (tape_ctrl_window_t * const tcw, /* TOHv4.4 */
                                bool const need_lock,
                                bool const need_unlock,
                                bool const strip_activated);
void tapectrl_to_gui_msg_overclock (tape_ctrl_window_t * const tcw, /* TOHv4.4 */
                                    bool const need_lock,
                                    bool const need_unlock,
