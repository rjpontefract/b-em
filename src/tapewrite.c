...
static int tape_write_data_1200th  (tape_state_t * const ts,
    ...
    if (iv_list_inout != NULL) {
        e = tape_interval_list_send_1200th (iv_list_inout, value, false, 0); /* false = no free on error; 0 = no &120 markers */
        if (TAPE_E_OK != e) { return e; }
    }
#endif
    ...
    if ( silent ) {
        ...
        for (i=0; (i < num_1200ths); i++) { /* TOHv4: tones_per_bit loop */
            ...
            if (record_is_pressed) {
                (t->tallied_1200ths)++;
#ifdef BUILD_TAPE_TAPECTRL
                e = tape_interval_list_send_1200th(iv_list_inout, 'S', false, 0); /* no free on error; no &120 markers */
#endif
            }
        }
        /* TOHv4.3 */
...
static int wp_bitclk_accumulate_leader (tape_state_t         * const t,
    ...
    for (i=0; i < num_1200ths; i++) {
         ...
         * the tapectrl. */
        if (record_is_pressed) { /* && tapectrl_opened) {*/
            e = tape_interval_list_send_1200th(iv_list_inout, 'L', false, 0); /* no free on error; no markers */
        }
        if (TAPE_E_OK != e) { return e; }
#endif
    }
