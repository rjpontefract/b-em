...
typedef struct tape_interval_marker_s { /* TOHv4.4 */
    int32_t pos_1200ths;
    /* TODO: marker text */
    /* NOTE that it will be necessary to clone this 'text' in tape_interval_list_clone(),
     * so don't forget to add that too: */
    /* char *text; */
} tape_interval_marker_t;
...
typedef struct tape_interval_s {
    ...
    /* must be freed: */
    tape_interval_marker_t *markers; /* TOHv4.4 */
    int32_t num_markers_fill;          /* TOHv4.4 */
    int32_t num_markers_alloc;         /* TOHv4.4 */
} tape_interval_t;
