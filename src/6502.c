...
/* new in TOHv4 */
static int tape_2mhz_main (tape_state_t * const ts,
                           tape_vars_t  * const tv,
                           ACIA         * const acia,
                           int            const num_cycles,
                           float          const tapenoise_volume_fraction,
...
static int tape_2mhz_handle_tape (tape_state_t * const ts,
                                  tape_vars_t  * const tv,
                                  ACIA         * const acia,
                                  int            const num_cycles,
                                  float          const tapenoise_volume_fraction,
...
static int tape_2mhz_handle_rs423_motor1 (tape_state_t * const ts,
                                          tape_vars_t  * const tv,
                                          ACIA         * const acia,
                                          int            const num_cycles,
...
static void tape_2mhz_handle_rs423_motor0 (tape_state_t * const ts,
                                           int const num_cycles); /* TOHv4.1-rc9 */
/* TOHv4 */
/* Simulates the 16/13 MHz clock generated by the serial ULA. */
static int
ula_2mhz_clock_for_tape (int32_t   const ula_rx_thresh_ns,
                         int32_t   const ula_tx_thresh_ns,
                         int32_t   const ula_rx_divider,
                         int32_t   const ula_tx_divider,
                         bool    * const fire_acia_rxc_out,
                         bool    * const fire_acia_2txc_out,
                         bool    * const fire_dcd_out,
                         int32_t * const rx_ns_inout,
                         int32_t * const tx_ns_inout,
                         int32_t * const dcd_2mhz_counter_inout);
...
static void tape_poll_2mhz (tape_state_t *ts,
                            ...
                            bool const record_activated) {
    int e;
    e = tape_2mhz_main (ts, tv, acia, num_cycles, tapenoise_volume_fraction, record_activated);
    /* FIXME? Should ( TAPE_E_OK == ts->prior_exception ) the old 'disabled_due_to_error' check
    *        be moved inside tape_handle_exception()? */
    if ( (TAPE_E_OK != e) && ( TAPE_E_OK == ts->prior_exception ) ) {
 ...
 */
static int
ula_2mhz_clock_for_tapenoise(int32_t * const tapenoise_counter_inout,
                             bool    * const fire_tapenoise_out) {
    ...
    if ((*tx_ns_inout) > ula_tx_thresh_ns) {
        *fire_acia_2txc_out = 1;
        ...
        /* fudge potential situation where we would have to fire twice
           (this fudge is what causes ULA /1 case to diverge from the hardware a bit) */
        if ((*tx_ns_inout) > ula_tx_thresh_ns) {
            *tx_ns_inout = 0;
        }
    }
...
static int tape_2mhz_main (tape_state_t * const ts,
    ...
    if (tv->tapectrl.inhibited_by_gui) { return TAPE_E_OK; } /* TOHv4.3 */
#endif
    
    throw_tape_eof = false;
    e = TAPE_E_OK;

    if (ts->ula_ctrl_reg & 0x40) { /* RS423 mode */
        if (ts->ula_motor) {
            /* RS423 with *MOTOR 1 */
            e = tape_2mhz_handle_rs423_motor1 (ts, tv, acia, num_cycles, tapenoise_volume_fraction, tv->record_activated, &throw_tape_eof);
        } else {
            tape_2mhz_handle_rs423_motor0 (ts, num_cycles);
        }
    } else {
        /* tape mode */
        e = tape_2mhz_handle_tape (ts, tv, acia, num_cycles, tapenoise_volume_fraction, &throw_tape_eof);
    }
    ...
    for (i=0; (i < num_cycles) && (TAPE_E_OK == e); i++) {
        ...
        if (fire_tapenoise && (tapenoise_volume_fraction > TAPENOISE_VOLUME_MINIMUM)) { /* 1201.9 Hz */
#ifdef BUILD_TAPE_TAPECTRL
            tapenoise_play_ringbuf(tapenoise_volume_fraction);
#else
            tapenoise_play_ringbuf(tapenoise_volume_fraction /* TAPENOISE_VOLUME_DEFAULT */);
#endif
        }
    }

    if (TAPE_E_OK != e) { return e; } /* TOHv4.3 */

    /* TOHv4.1: suppress EOF in record mode */
    return ((throw_tape_eof && ! tv->record_activated) ? TAPE_E_EOF : e);
    
}

/* new in TOHv4.1-rc9 */
static void tape_2mhz_handle_rs423_motor0 (tape_state_t * const ts, int const num_cycles) {
    int i;
    for (i=0; (i < num_cycles); i++) {
        if ( ts->ula_rs423_taperoll_2mhz_counter >= TAPE_1200TH_IN_2MHZ_INT ) { /*(128*13) ) {*/
            ...
            /* TOHv4.4: need to track how long the motor has been off, in order to
             * distinguish between it being just pulsed off after a block, and it being
             * switched off decisively. */
            if (ts->ula_motor_stopped_ticks_1200 < TAPE_MIN_TIME_MOTOR_OFF_TO_CLR_STRIP_HOLDOFF_1200THS) {
                ts->ula_motor_stopped_ticks_1200++;
            }
        }
...
/* TOHv4.1: As long as tape mode is selected in the ULA, this
 *          function is polled regardless of motor state. */
static int tape_2mhz_handle_tape (tape_state_t * const ts,
                                  tape_vars_t  * const tv,
                                  ACIA         * const acia,
                                  int            const num_cycles,
    ...
    int e,j;
    bool my_sound_tape; /* TOHv4.4 */
    
    e = TAPE_E_OK;
    
    my_sound_tape = tapenoise_volume_fraction > TAPENOISE_VOLUME_MINIMUM;

    /* if (ts->disabled_due_to_error) { */ /* TOHv4.3 */
    if ( TAPE_E_OK != ts->prior_exception ) { /* TOHv4.3 */
        return TAPE_E_OK;
    }

    for (j=0; (TAPE_E_OK == e) && (j < num_cycles); j++) {
    
        bool fire_ula_rxc, fire_ula_2txc, fire_dcd_tick;
        ...
        e = ula_2mhz_clock_for_tape (ts->ula_rx_thresh_ns,
                                     ...
                                     &fire_dcd_tick,         /* fire DCD tick? every ~211 us */
                                     &(ts->ula_rx_ns),              /* counter updated */
                                     &(ts->ula_tx_ns),              /* counter updated */
                                     &(ts->ula_dcd_2mhz_counter));  /* counter updated */

        /* 2. handle RX */
        if ((TAPE_E_OK == e) && fire_ula_rxc && ts->ula_motor) { /* TOHv4.1: gated by ula_motor now */
            e = serial_rxc_clock_for_tape (ts,
                                           tv,
                                           acia,
                                           my_sound_tape && ! tv->record_activated,
                                           tv->record_activated,
                                           throw_eof_out);
        }
        ...
        if ((TAPE_E_OK == e) && fire_ula_2txc) {
            ...
            if (TAPE_E_ACIA_TX_BITCLK_NOT_READY == e) {
                /* if fire_ula_2txc was true then this shouldn't have happened */
                log_warn("6502: BUG: e == TAPE_E_ACIA_TX_BITCLK_NOT_READY");
                return TAPE_E_BUG;
            } else if (TAPE_E_EOF == e) {
                /* TOHv4.3-a4: There should never have been EOFs generated write-side.
                 *             Ban this sick filth. */
                log_warn("6502: BUG: TX-side tape EOF is not allowed");
                return TAPE_E_BUG;
            }
#endif
        }
        ...
        if (fire_dcd_tick) { serial_handle_dcd_tick(ts, tv, acia); }
...
static int tape_2mhz_handle_rs423_motor1 (tape_state_t * const ts,
    ...
    for (j=0;
        ...
        /* while tape is rolling, discard each 1200th,
           ...
           update playback elapsed */
        if ( ts->ula_rs423_taperoll_2mhz_counter >= TAPE_1200TH_IN_2MHZ_INT ) { /*(128*13) ) {*/
            e = tape_rs423_eat_1200th (ts, tv, acia, record_activated, emit_tapenoise, throw_eof_out);
            if (TAPE_E_OK != e) { return e; }
        }

        /* Internal tape DCD logic and run-in detection etc. should continue
           to function while RS423 mode is being used. (MOTOR 0 inhibits DCD logic
           but RS423 mode doesn't.)
