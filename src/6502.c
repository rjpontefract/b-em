...
static void tape_poll_2mhz (tape_state_t *ts,
                            ...
                            bool const record_activated) {
    int e;
    e = tape_2mhz_main (ts, tv, acia, num_cycles, my_sound_tape, record_activated);
...
static int tape_2mhz_main (tape_state_t * const ts,
    ...
    if (ts->ula_ctrl_reg & 0x40) { /* RS423 mode */
        if (ts->ula_motor) {
            /* RS423 with *MOTOR 1 */
            e = tape_2mhz_handle_rs423_motor1 (ts, tv, acia, num_cycles, my_sound_tape, tv->record_activated, &throw_tape_eof);
        } else {
...
static int tape_2mhz_handle_tape (tape_state_t * const ts,
    ...
    for (j=0; (TAPE_E_OK == e) && (j < num_cycles); j++) {
        ...
        bool fire_ula_rxc, fire_ula_2txc, fire_dcd;
