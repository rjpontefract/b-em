...
wp1_4k8_init_blank_tape_if_none_loaded (uint8_t *filetype_bits_inout);
                                        /*uint8_t record_activated);*/ /* TOHv4.1: removed */
...
/*                                        uint8_t record_activated) {*/
    ...
    *filetype_bits_inout = TAPE_FILETYPES_ALL;
    ...
    /*gui_set_record_mode(1);*/ /*record_activated);*/
    return TAPE_E_OK;
}
...
/*printf("tape_flush_pending_piece: w_accumulate_silence_ns = %lld\n", ts->w_accumulate_silence_ns);*/
    ...
    /* TOHv3.2: fixed memory leak: never do this;
...
static int tape_write_leader (tape_state_t *t, uint32_t num_1200ths) {
    ...
    if (t->filetype_bits & TAPE_FILETYPE_BITS_UEF) {
        ...
        e = uef_store_chunk(&(t->uef), num_2400ths, 0x110, 2, 0);  /* TOHv4.2: dummy offset */
        if (TAPE_E_OK != e) { return e; }
    }
    ...
    if (t->filetype_bits & TAPE_FILETYPE_BITS_UEF) {
        if (silence112) {
            ...
            for ( rem = num_112_cycs; rem > 0; rem -= 65535 ) {
              ...
              e = uef_store_chunk(&(t->uef), buf, 0x112, 2, 0);  /* TOHv4.2: dummy offset */
              if (TAPE_E_OK != e) { return e; }
            }
        } else {
            /* chunk &116 */
            ...
            u.f = len_s;
            e = uef_store_chunk(&(t->uef), u.b, 0x116, 4, 0); /* TOHv4.2: dummy offset */
            if (TAPE_E_OK != e) { return e; }
        }
    }
...
int tape_generate_and_save_output_file (tape_state_t *ts, /* TOHv4.2: pass this in */
    ...
    e = tape_flush_pending_piece (ts, acia_or_null, silence112);
    if (TAPE_E_OK != e) { return e; }
...
//ts->silence_skip_1200ths = 0;  /* TOHv4.2 */
            return TAPE_E_EOF;
        }
        if (e != TAPE_E_OK) { return e; }
         ...
         * If we have only seen a very short duration of leader 'L' so far,
         ...
         * We go back to using 'L' once we've inserted a few 1200ths
         * to make MOS happy. If enabled, the leader skip code can then do
         * its thing with impunity.
         *
...
//    ts->silence_skip_1200ths = 0;
//} else {
//    (ts->silence_skip_1200ths)++;
//    if (ts->silence_skip_1200ths < 10) {
//        break;
//    }
//}
...
int findfilenames_new (tape_state_t *ts,
    ...
    tape_handle_exception (&tclone, NULL, e, tape_vars.testing_mode & TAPE_TEST_QUIT_ON_EOF, 0, 0);
...
void tape_handle_exception (tape_state_t *ts,
                            ...
                            uint8_t eof_fatal,
                            uint8_t err_fatal,
                            uint8_t alter_menus) { /* 0 if called from findfilenames_new */
    int e, sx;
    uint8_t tape_broken;
    ...
    if ( tape_broken ) {
        ...
        tape_state_finish(ts, alter_menus); /*  1 = alter menus  */
        if (tv != NULL) {
...
int tape_rewind_2 (tape_state_t *t) {
    int e;
    e = TAPE_E_OK;
...
void tape_init (tape_state_t *ts, tape_vars_t *tv) {
    /*tape_state_init (ts, tv, 0, 0);*/             /* TOHv4.2 */
...
//printf("*our_filetype_bits_inout = %x, c->filetype_bits = %x\n",
...
void tape_start_motor (tape_state_t *ts, uint8_t also_force_dcd_high) {
    ...
    /* TOHv4.2: reset this to ensure another half a second
...
int tape_rs423_eat_1200th (tape_state_t *ts,
     ...
     * block "motor off" breaks, so eventually you would get dropouts. */
...
int tape_fire_acia_rxc (tape_state_t *ts,
    ...
    /* TOHv4.1: this gate is instead applied to serial_rxc_clock_for_tape call in 6502.c */
...
strip = tv->strip_silence_and_leader;
    ...
    if (832000 == ns_per_bit) {
        ...
        /* 1201.92 baud */
        e = tape_tone_1200th_from_back_end (strip,
                                            ts,
