...
/* WARNING: on error, caller is expected to clean up "out", e.g. calling tape_state_finish() */
...
int findfilenames_new (tape_state_t * const ts,
    ...
    while ( TAPE_E_OK == e)  {
        ...
        for (n=0; (n < limit) && (TAPE_E_OK == e); n++) {
            ...
            e = tape_1200th_from_back_end (false, /* don't strip silence and leader */
                                           ...
                                           NULL);                             /* TOHv4.4: don't need list of time markers */
    ...
    /* if timestamps are to be printed, then print a total at the end */
#ifdef BUILD_TAPE_CAT_TIMESTAMPS
    tape_get_duration_1200ths(ts, &duration_1200ths);
    to_hours_minutes_seconds (duration_1200ths, &hrs, &mins, &secs);
    sprintf(s, "Total %d:%02d:%02d", hrs, mins, secs);
    cataddname("");
    cataddname(s);
#endif
...
int tape_load_successful (tape_state_t * const ts,
    ...
    /* TOHv4.3: check removed: record mode on load is a valid configuration
     * if -tape and -record provided on CL */
    /*
...
int tape_start_motor (tape_state_t * const ts,
    ...
    /* TOHv4.4: bugfix for strip mode not working properly.
     * Pulsing the cassette motor off at the end of each block
     * was resetting holdoff to zero. Holdoff is only supposed
     * to be reset to 0 if the motor is turned off definitively
     * rather than just pulsed off. So we'll have to note for how
     * long the motor was off. Annoying. */
    ...
    /* TOHv4.4 */
    ts->ula_motor_stopped_ticks_1200 = 0;
...
int tape_rs423_eat_1200th (tape_state_t * const ts,
    ...
    if ( ! record_activated ) {
        e = tape_1200th_from_back_end (false, ts, false, false, &tone, &elapsed_or_minus_one, NULL);
        if (TAPE_E_EOF == e) {
            *throw_eof_out = true;
    ...
    if ( record_activated ) {
        ...
        /* playback mode */
        /* increments ts->tallied_1200ths when passed -1 */
    ...
    if ('1' == tone) {
        if (ts->rs423_leader_detect > TAPE_CRUDE_LEADER_DETECT_1200THS /* =170 */) {
            tone = 'L';
        } else {
