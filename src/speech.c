...
void speech_init(void)
{
    ...
    if (dir) {
        const size_t size = 16 * SPEECH_ROM_SIZE;
...
/*
 * Called from sound_poll() once per 125kHz output sample.
 * Advances the TMS5220 at 8kHz (every 125000/8000 = 15.625 output samples)
 * and returns a sample to mix into the main sound buffer.
 */
int16_t speech_mix(void)
{
    if (!speech_initialised)
        return 0;
    speech_mix_frac += FREQ_SPEECH;       /* 8000 */
    if (speech_mix_frac >= FREQ_SO) {     /* 125000 */
        speech_mix_frac -= FREQ_SO;
        speech_process(&speech_mix_sample, 1);
    }
    return speech_mix_sample;
}

void speech_poll(int c)
{
