...
// lovebug end
    ...
    "-tapetest <n>   - bits: 1=quit @ EOF | 2=quit @ err | 4=cat @ boot | 8=cat @ end\n" /* TOHv3, v4   */
...
/* FIXME: client code wishing to shut down either calls
   this function or just sets quitting=1 directly; there is
   no consistency. It should be one or the other.
   
   Suggest also that this function should now take a shutdown
   code as an argument. */
...
void set_shutdown_exit_code (uint8_t c) {
    /* hack: prevent a later exit code from overwriting an earlier one
       (yes, this actually happens, tape EOF scribbles tape error) */
