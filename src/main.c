...
#ifdef BUILD_TAPE_TAPECTRL
    "-tapeuiscale n  - set scale at which to display tape control window (0.5 - 1.5)\n"  /* TOHv4.3, v4.4 */
    /* -tapeuiresize removed; always allow it */
#endif
    "-expire n       - quit, with predictable exit code, after n emulated seconds\n"     /* TOHv3.3 */
    "-tapenopbp      - do not filter out phantom blocks on tape load\n"
    "-nosavecfg      - do not write configuration file on shutdown\n"                    /* TOHv4.4 */
    "\n";
 ...
 * Just always do this, even if we're on an old Intel Mac
 * which doesn't crash. */
#ifdef __APPLE__
    ...
    tape_ctrl_window_t *tcw;         /* TOHv4.4 */
    bool strip_cli, wav_cosine_cli, tape117_cli, save_no_origin_cli, tape112_cli, permit_phantoms_cli;  /* TOHv4.4 */
    ...
    /* TOHv4.4: temporary config variables from CL */
    strip_cli = false;
    wav_cosine_cli = false;          
    tape117_cli = false;
    save_no_origin_cli = false;
    tape112_cli = false;
    permit_phantoms_cli = false;
            ...
            case OPT_GROUND:
                if (*arg == '-') {
                    ...
                    /* begin TOHv4.2 merge ... */
                        ...
                        /* this is temporarily written to a separate variable,
                         * so it can override the value that will be loaded from
                         * the config a bit later */
                        strip_cli = true; /* TOHv4.4 */
                        ...
                        */
                        ...
                        tape_set_record_activated(&tape_state,
                                                  ...
                                                  tcw_opened);
                    /* end TOHv4.2 merge */
                    } else if (!strcasecmp(arg, "nosavecfg")) { /* TOHv4.4 */
                        tape_vars.no_save_config_on_shutdown = true;
    ...
    /* TOHv4.4: allow various command-line switches to override config: */
    if (strip_cli) {
    ...
    if (save_no_origin_cli) {
        tape_vars.save_do_not_generate_origin_on_append = true;
    }
    if (tape112_cli) {
        tape_vars.save_prefer_112 = true;
    }
    if (permit_phantoms_cli) {
        tape_vars.permit_phantoms = true;
    }
    
//~ printf("save_prefer_112 = %u\n", tape_vars.save_prefer_112); exit(0);
    ...
    /* TOHv4.4: moved after allegro init and tapectrl init,
     * so that it can both alter menu checkboxes, and lock
     * the tapectrl mutex */
    tapenoise_init(queue, tcw);
...
void main_close()
{
    ...
    if ( ! tape_vars.no_save_config_on_shutdown ) { /* TOHv4.4: prevent tests scribbling config file */
        config_save();
    }
