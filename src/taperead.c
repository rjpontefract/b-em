...
//~ printf("ts->strip_silence_and_leader_holdoff_1200ths = %d\n", ts->strip_silence_and_leader_holdoff_1200ths);
    ...
    if ( (832000 == ns_per_bit) || (52000 == ns_per_bit) ) {
        ...
        if (fire) { /* every 1200th, for 1200 and 19200 baud modes */
            ...
            e = tape_1200th_from_back_end (strip,
                                           ...
                                           NULL);                      /* TOHv4.4: markers list not needed */
        ...
        for (m=0; m < TAPE_TONES300_BUF_LEN; m++) {
            ...
            e = tape_1200th_from_back_end (tv->strip_silence_and_leader,
                                            ...
                                            &tone,
                                            &time_1200ths_or_minus_one,   /* TOHv4.3 */
                                            NULL);                        /* TOHv4.4: markers not needed */
            if (TAPE_E_EOF == e) {
                *throw_eof_out = true;
                e = TAPE_E_OK;
            }
...
int tape_1200th_from_back_end (        bool   const strip_silence_and_leader,              /* turbo */
                               tape_state_t * const ts,
                                    ...
                                    int32_t * const num_markers_out_or_null) {             /* TOHv4.4 */

    int e;
    bool inhibit_start_bit_for_phantom_block_protection; /* TOHv3.2 */
    
    if (NULL != num_markers_out_or_null) {
        *num_markers_out_or_null = 0;
    }
    ...
    *tone_1200th_out = '\0';
    ...
    do { /* TOHv3.2: only loops back if (strip_silence_and_leader) */
        ...
        e = tape_read_1200th (ts, 0, tone_1200th_out, updated_elapsed_1200ths_out_or_null, num_markers_out_or_null);
        if (TAPE_E_EOF == e) {
            *tone_1200th_out = 'S';
        ...
        if (('0' == *tone_1200th_out) || ('S' == *tone_1200th_out)) {
            ...
            /* So, make sure enough '1's escape without an 'L'
             * to satisfy the fast-DCD condition. (If strip mode
             * is disabled, 'L' is not treated specially, so it
             * doesn't matter when slow-DCD mode switches from '1' to 'L';
             * just use the fast mode value for this mode too.) */
        } else if ( ('L' == *tone_1200th_out)
                    && (ts->leader_skip_1200ths < TAPE_BLIPTICKS_BEFORE_DCD_BLIP_FAST /*10*/)) {
...
int tape_read_1200th (tape_state_t * const ts,
                      ...
                      int32_t * const num_markers_out_or_null) {           /* TOHv4.4: the metadata exfiltrations have begun */
    ...
    if (num_markers_out_or_null != NULL) {
        *num_markers_out_or_null = 0; 
    }
    ...
    if (TAPE_FILETYPE_BITS_UEF & ts->filetype_bits) {
        ...
        if ((TAPE_E_OK == e) || (TAPE_E_EOF == e)) { /* throws EOF */
            for (m=0; m < meta_len; m++) {
                if (meta[m].type != 0x117) { /* baud rate is currently used */
                    log_info("uef: unused metadata, chunk &%x\n", meta[m].type);
                }
                /* TOHv4.4 */
                if ((num_markers_out_or_null != NULL) && (0x120 == meta[m].type)) { /* position marker */
                    (*num_markers_out_or_null)++;
                    /* TODO: actually return the labels themselves.
                     * There may be up to UEF_MAX_METADATA of them.
                     * Or just malloc the precise amount. */
                }
            }
        ...
        e = tibet_read_bit (&(ts->tibet),
                            &(ts->stray_2400th),      /* TOHv4.4: may be modified */
                            0,                        /* always 1/1200th tones */
                            initial_scan_mode,        /* TOHv4.3 */
                            value_out_or_null,
                            updated_elapsed_1200ths_out_or_null);
    } else if (TAPE_FILETYPE_BITS_NONE == ts->filetype_bits) {
        e = TAPE_E_EOF;
    } else {
        log_warn("tape: BUG: unknown internal filetype: &%x", ts->filetype_bits);
        return TAPE_E_BUG;
    }

    if (TAPE_E_EOF == e) {
        if ( ! ts->tape_finished_no_emsgs ) {
            if (initial_scan_mode) { /* TOHv4.3: */
                tape_get_duration_1200ths(ts, &d);
        ...
        ts->tape_finished_no_emsgs = 1; /* suppress further messages */
    } else {
        ts->tape_finished_no_emsgs = 0;
    }
