...
/* TOHv4.3 */
static int get_time_interval_for_piece (tape_state_t    * const ts,       /* data not const -- TIBET might modify it */
...
/* printf("SEEK: wanted %d, hit %d\n", time_1200ths_wanted, interval.start_1200ths); */
    ...
    for (t = interval.start_1200ths ;
        ...
        e = tape_1200th_from_back_end (false, ts, false, false, &tc, NULL, NULL);
...
//~ printf("interval_out: start_1200ths = %d, pos_1200ths = %d\n", interval_out->start_1200ths, interval_out->pos_1200ths);
    return TAPE_E_OK;
}
...
int tapeseek_run_initial_scan (        tape_state_t         * const ts,
    ...
    do {
        int32_t num_markers; /* TOHv4.4: count chunk &120s */
        
        tone = '\0';
        num_markers = 0; /* TOHv4.4 */
        
        e = tape_read_1200th (ts, true, &tone, NULL, &num_markers); /* initial-scan mode! build time map on ts */
...
//~ printf("%c", tone);
            e = tape_interval_list_send_1200th (iv_list_inout, tone, true, num_markers); /* free on error */
        }
#endif
...
//~ printf("finalo: type %u, start_1200ths %d, pos_1200ths %d\n", iv_list_inout->decstate.wip.type, iv_list_inout->decstate.wip.start_1200ths, iv_list_inout->decstate.wip.pos_1200ths);
    ...
    if ((iv_list_inout != NULL) && (iv_list_inout->fill > 0)) {
        ...
        np=0;
        get_num_pieces(ts, &np); /* num UEF chunks, or TIBET num spans, or CSW num pulses */
        if (np>0) {
         ...
         * In fact delta should always be 0; the fact that it is sometimes 1
         * is a bug, but we'll tolerate it for now.
         * 
         * See the test 223__Seof_Ferr_obiwan_duration_mismatch.tibet, which
         * will generate this 1-off condition. Certain CSWs and UEFs cause it too.
         * 
         * The TIBET is very simple:
         * 
         * tibet 0.5
         * leader 3
         * silence 0.005     # Six 1200ths
         * 
         * The discrepancy is caused by the some of those 3 leader 1200ths
         * disappearing at the very start of the stream; it doesn't seem
         * possible to build a test case based on this which causes |delta| > 1,
         * or at least I haven't found a way of doing it yet.
         */
        if (1==delta) {
