...
#define TAPE_MIN_TIME_MOTOR_OFF_TO_CLR_STRIP_HOLDOFF_1200THS 100
...
typedef struct tape_state_s {
    ...
    /* WRITING (w_...) : */
    bool w_must_end_data; /* data pending, e.g. on w_uef_tmpchunk */
    ...
    bool ula_motor;       /* TOHv3.3: moved here from tape_vars_t */
...
#define TAPE_TONES300_BUF_LEN 4
    ...
    char stray_2400th; /* TOHv4.4: fix dropping tapetime on encountering singleton 2400th cycles */
        
    /* TOHv4.4: motor must be switched off properly in order to
     *          reset the holdoff rather than just pulsed, so
     *          need to measure how long the motor is off. */
    int32_t ula_motor_stopped_ticks_1200;
...
typedef struct tape_vars_s {
    ...
    bool no_save_config_on_shutdown; /* TOHv4.4 */
    ...
    double flash_last_toggled_time_s; /* for play indicator */
    bool flash_currently_illuminated;
