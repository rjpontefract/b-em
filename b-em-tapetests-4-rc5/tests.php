<?php
  ...
  define ("TESTS_VERSION", "4-rc5");
  define ("TESTS_DATE", "23rd February 2025");
  ...
  class TestsState {
    ...
    static function usage(string $argv0) {
      ...
      print "Usage:\n".
            ...
            "    ".str_pad(CLI_MODE_BRIEF, $pad)." : synthetic tests only, no protected games (~3 minutes)\n".
            "    ".str_pad(CLI_MODE_FULL,  $pad)." : as ".CLI_MODE_BRIEF.", plus protected games (~40 minutes)\n".
...
//for N in $(cat list.txt | grep state | tr ' ' ^) ; do N="$(echo $N | tr ^ ' ')" ; A=$(echo $N | sed -e 's/^run.\+[$]p[.]"\([^"]*\)".*$/\1/g') ; B="$(echo $A | sed 's/^\([0-9]\{3\}\)__.\+uef$/\1/g' | sed s/^0*''//g)" ; C=$(printf %03d $[ $B + 22 ]) ; D="`echo $N | sed -e 's/[0-9]\{3\}__/'$C'__/g'`" ; echo $D ; done
  ...
  run ($st, "", $p."060__ERR__0115_phase_change__060__TOO_LARGE.uef",                             "", $grp, X_ERR, 7, "Chunk &115, phase = 361", $skip, array(), $xs);
  ...
  run ($st, "", $p."065__ERR__0117_baud__1065__ILLEGAL_BAUD.uef",                                 "", $grp, X_ERR, 7, "Chunk &117, 1199 baud", $skip, array(), $xs);
...
//  die();
  ...
  $d = "b 4000\nbx 0\npaste ?&4000=&12|MCH.\"600\"|M|MCALL &4000|M\nc\n\n\n";
  $u = run_without_testing ($st, $d, "", "-disc ".$p."weirdbaud.ssd -record -tapesave ".PATH_TIBET_OPFILE, 0, PATH_TIBET_OPFILE, $skip, $xs2);
...
//  die();
  ...
  // This gives us a chance for the demo mode to run for a while, which will give a
  // chance to make sure the copy protection doesn't trigger, because it can trigger
  // belatedly (i.e. minutes after loading completed) for certain ACIA mis-implementations!
...
/*
  ...
  function prepare_command_line (string $tape_to_load, int $tapetest_bits, string $extra_args, bool $verbose, string $path_sep) {
...
//    if ((strlen($extra_args) > 0) && ($extra_args[strlen($extra_args)-1] != " ")) { $extra_args .= " "; }
    ...
    // TOHv4-rc4: now send '-tapetest 0' in order to force
    // memory display window suppression even if no tapetest
    // quit condition was specified
    //if ($tapetest_bits != 0) {
...
//    if ((strlen($extra_args)>0) && ($extra_args[strlen($extra_args) - 1] != " ")) { $extra_args .= " "; } // append space to extra_args if needed
    ...
    $cl = prepare_command_line($tape_to_load, $tapetest_bits, $extra_args, $ts->verbose, $path_sep);
